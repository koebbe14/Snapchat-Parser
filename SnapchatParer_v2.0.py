...
# Blur settings (matching kik_parser.py)
...
def configure_table_optimal_sizing(table, headers, table_name="table", settings=None):
    ...
    # Set min/max section sizes globally (like kik_parser.py)
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            if header == "Date":
                ...
                return str(msg.get('conversation_id', ''))
...
class MediaThumbnailDelegate(QStyledItemDelegate):
    ...
    def sizeHint(self, option, index):
        """Return size hint for the thumbnail cell - this determines row height (matching kik_parser.py)."""
        media_info = index.data(Qt.UserRole)
        if media_info and isinstance(media_info, dict):
            # Check for multiple media paths
            content_paths = media_info.get('content_paths', [])
...
class ClickableThumbnail(QLabel):
    ...
    def contextMenuEvent(self, ev):
        ...
        if action == toggle_action:
            ...
            effect.setBlurRadius(10)  # Match kik_parser.py blur radius
...
class MessageViewerDialog(QDialog):
    ...
    def populate(self, message_indices):
            ...
            for msg_index in message_indices:
                ...
                col_data = {
					"Conversation ID": str(msg.get('conversation_id', '')),
                    "Conversation Title": str(msg.get('conversation_title', '')),
                            ...
                            if extracted and os.path.exists(extracted):
                                ...
                                if thumb and os.path.exists(thumb):
                                    ...
                                    if self.parent.blur_all:
                                        ...
                                        eff.setBlurRadius(10)  # Match kik_parser.py blur radius
...
class ExportOptionsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        sort_layout = QHBoxLayout()  # NEW: Sort option like Kik
        ...
        fields_group = QGroupBox("Fields to Include")  # NEW: Fields selection like Kik
        ...
        try:
            ...
            for file_idx, (zip_with_conv, internal_conv) in enumerate(conv_files):
                ...
                try:
                    ...
                    df = df[df['content_type'].notna() & df['conversation_id'].notna()]
                    ...
                    for row_tuple in df.itertuples(index=True):
                        ...
                        # OPTIMIZED: Convert namedtuple to dict - itertuples uses positional access (_1, _2, etc.)
                        # More efficient: use iloc directly for this row
                        msg = df.iloc[idx].to_dict()
...
class SnapParserMain(QMainWindow):
    ...
    def populate_selector(self, progress_dialog=None, progress_start=0, progress_end=100, phase_num=None):
        ...
        sorted_conv_ids = sorted(self.conversations.keys(), key=lambda x: self.conversations[x][0], reverse=True)
    ...
    def populate_message_table(self, all_messages, conversations, conv_id,
        ...
        # Reapply default column widths and resize rows after a short delay
        # Use a longer delay to ensure it happens after dialog closes, and make it non-blocking
        QTimer.singleShot(100, lambda: (
            self._reapply_default_column_widths(),
    ...
    def _resize_rows_with_thumbnails(self):
        """Set appropriate row heights for rows with thumbnails (similar to kik_parser.py)."""
        model = self.message_table.model()
        if not model:
            return
    ...
    def perform_export(self, options):
        ...
        # 5. Media handling with blur (like Kik)
        if 'Media' in selected_fields and options['format'] == 'HTML':
        ...
        if options['format'] == 'HTML':
            ...
            <option value="Of Interest">Of Interest</option>
            </select>
            ...
            function filterTable() {
                ...
                for (var j = 0; j < headers.length; j++) {
                    var headerText = headers[j].innerHTML.trim();
                    if (headerText === "Tags") {
                        tags_col = j;
                ...
                // Use tr[data-conversation] to get all rows with conversation data
                var rows = table.querySelectorAll('tr[data-conversation]');
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    // Tag filter
                    if (show && tag && tags_col !== -1 && td.length > tags_col) {
                        var tags_text = td[tags_col].innerHTML;
                        show = show && tags_text.includes(tag);
                    }
    ...
    def apply_column_config(self):
        ...
        for i in range(header.count()):
            header_text = self.message_table.horizontalHeaderItem(i).text()
